...
fun main() {
    ...
    students.forEach { student ->
        ...
        student.studentPair = Pair(one, two)
    }
    ...
    while (failure.isNotEmpty()) {
        ...
        students[idx].studentPair.let { idxes -> // 해당 학생이 선택한 두명의 학생은 학생 a에게 선물을 받지 못함 -1
            if (--students[idxes.first].numOgGift == 1) failure.add(students[idxes.first]) // 받은 선물의 갯수가 2 - 1 = 1이 된 경우, failure에 새롭게 추가
            if (--students[idxes.second].numOgGift == 1) failure.add(students[idxes.second])
        }
    }
